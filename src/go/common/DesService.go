...
var key = []byte("asdsfdcc")

//des的cbc模式加密函数
//src ：明文
//key :密钥，大小为8byte
func DesEncrypt_CBC(src []byte) (string,error){  //传递两个参数,src为需要加密的明文，返回[]byte类型数据
...
func DesDecrypt_CBC(src  string) (string,error){
	...
	dst:=make([]byte,len(byteSrc))
	blockMode.CryptBlocks(dst,byteSrc)
	//5:去掉最后一组填充数据
